...
export function PlanWidget() {
    ...
    return (
        <div className="mx-4 mb-6 bg-gradient-to-br from-[#1C1C1E] to-[#121212] border border-white/10 rounded-2xl p-5 relative overflow-hidden">
            {/* Background Accent */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#F20000]/5 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2 pointer-events-none" />
            ...
            <div className="flex items-start justify-between relative z-10">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        {data.plan !== 'studio' && (
                            <span className="bg-[#F20000]/10 text-[#F20000] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                                {data.billing_cycle}
                            </span>
                        )}
                    </div>
                    {data.client_limit ? (
                        <p className="text-white/50 text-sm">
                            <span className={data.show_upgrade_prompt ? "text-[#F20000] font-semibold" : "text-white/80 font-medium"}>
                                {data.active_clients}
                            </span>
                ...
                {data.show_upgrade_prompt && (
                    <button className="flex items-center gap-1.5 bg-[#F20000] hover:bg-[#D90000] text-white text-xs font-semibold px-3 py-2 rounded-lg transition-colors active:scale-95 shadow-[0_0_15px_rgba(242,0,0,0.3)]">
            ...
            {data.client_limit && (
                <div className="mt-4">
                    <div className="w-full bg-black/50 rounded-full h-1.5 overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all duration-1000 ${data.show_upgrade_prompt ? 'bg-[#F20000]' : 'bg-white/40'}`}
                            style={{ width: `${Math.min(data.utilisation_pct, 100)}%` }}
                        />
                    </div>
                    {data.show_upgrade_prompt && (
                        <p className="text-[#F20000] text-xs mt-2 font-medium">
