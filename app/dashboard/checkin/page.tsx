...
import { getCurrentCoachIdClient } from "@/lib/client-utils";
    ...
    if (step < total - 1) {
      ...
      startTransition(async () => {
        try {
          ...
          const coachId = await getCurrentCoachIdClient();
          await submitCheckin({
            coach_id: coachId,
            client_id: clientId,
            energy,
          ...
          if (error instanceof Error && error.message.includes('Unauthorized')) {
            router.push('/login?redirect=/dashboard/checkin' + (clientId ? `?clientId=${clientId}` : ''));
            return;
          }
          alert('Failed to submit checkin. Please try again.');
        } finally {
    ...
    return (
      <div className="flex-1 w-full bg-[#0A0A0A] text-white flex flex-col items-center justify-center p-8 min-h-screen font-sans">
        ...
        <button
          ...
          className="bg-[#E8001D] hover:bg-[#C20000] text-white px-12 py-4 rounded-xl text-base font-bold font-barlow tracking-widest uppercase transition-colors shadow-[0_4px_24px_rgba(232,0,29,0.18)]"
        >
          Done
        </button>
      </div>
    ...
    <div className="flex-1 w-full flex flex-col min-h-screen bg-[#0A0A0A] text-white font-sans overflow-hidden">
            ...
            {[0, 1, 2, 3, 4, 5, 6, 7].map(n => (
              <button
                ...
                className={cn(
                  ...
                  sessions === n
                    ? "bg-[#E8001D] border-[#E8001D] text-white shadow-[0_4px_16px_rgba(232,0,29,0.25)] scale-105"
      ...
      <div className="px-6 pb-8 pt-4 flex gap-3 bg-[#0A0A0A]">
        ...
        <button
          ...
          className="flex-[2] bg-[#E8001D] hover:bg-[#C20000] border-none rounded-xl p-4 text-white text-base font-bold font-barlow tracking-widest uppercase transition-colors shadow-[0_4px_24px_rgba(232,0,29,0.18)] active:scale-[0.98] disabled:opacity-50"
        >
