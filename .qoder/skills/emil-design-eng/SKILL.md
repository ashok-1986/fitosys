...
Proactive reference for common animation mistakes before they happen:

| Avoid | Why |
| ----- | --- |
| `transition: all` | Triggers layout recalc on every property; specify exact properties |
| `transform: scale(0)` | Nothing appears from nothing; start from `scale(0.95)` with opacity |
| `ease-in` for UI feedback | Feels sluggish; the user watches the slow start — use `ease-out` |
| Keyframes on rapidly-toggled UI | Cannot be interrupted mid-animation; use CSS transitions |
| Hardcoded pixel values in `translateY` | Breaks with dynamic content; use `translateY(100%)` (relative to element's own height) |
| Animation on keyboard actions | Makes repeated actions feel slow; keyboard shortcuts run 100+ times/day — never animate them |
| `transform-origin: center` on popovers | Popovers should scale from their trigger; use CSS variables from Radix/Base UI |
| Duration > 300ms on UI elements | Feels sluggish; UI animations should stay under 300ms |
| Hover animations without media query | Triggers false positives on touch devices; gate with `@media (hover: hover) and (pointer: fine)` |
| Elements all appearing at once | No stagger makes group entries feel mechanical; add 30-80ms delay between items |
...
> **Browser support:** `@starting-style` has baseline 2023 support (Chrome 117+, Safari 17.5+, Firefox 129+). For older browsers, continue using the `data-mounted` attribute pattern.
